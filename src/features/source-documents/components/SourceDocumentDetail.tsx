...
export function SourceDocumentDetail({ item, onOpen, onDownload, onReanalyse }: SourceDocumentDetailProps) {
  ...
  if (!item) {
    return (
      <section className="flex h-full min-h-0 flex-1 items-center justify-center rounded-2xl bg-white px-6 text-center text-[14px] font-medium text-[#6d7b93] shadow-[0_1px_2px_rgba(28,42,78,0.04)]">
  ...
  return (
    <section className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden rounded-2xl bg-white shadow-[0_1px_2px_rgba(28,42,78,0.04)]">
      ...
      <div className="mt-4 flex shrink-0 flex-wrap items-center gap-2 px-4 sm:px-5">
        <button
          type="button"
      ...
      <div className="mt-4 flex shrink-0 gap-4 border-b border-[#eef3f9] px-4 sm:px-5">
        {TABS.map((entry) => {
      ...
      <div className="min-h-0 flex-1 overflow-hidden px-4 py-4 sm:px-5">
        {tab === 'preview' ? (
          <SourceDocumentPreview document={item} />
        ) : (
